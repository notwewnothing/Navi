...
class MainActivity : FlutterActivity() {
    override fun configureFlutterEngine(flutterEngine: FlutterEngine) {
        ...
        MethodChannel(flutterEngine.dartExecutor.binaryMessenger, "navi/device_admin").setMethodCallHandler { call, result ->
            val componentName = ComponentName(this, DeviceAdminReceiver::class.java)
            val dpm = getSystemService(Context.DEVICE_POLICY_SERVICE) as android.app.admin.DevicePolicyManager
            when (call.method) {
                "isActiveAdmin" -> result.success(dpm.isAdminActive(componentName))
                "requestAdmin" -> {
                    val intent = Intent(android.app.admin.DevicePolicyManager.ACTION_ADD_DEVICE_ADMIN).apply {
                        putExtra(android.app.admin.DevicePolicyManager.EXTRA_DEVICE_ADMIN, componentName)
                        putExtra(
                            android.app.admin.DevicePolicyManager.EXTRA_ADD_EXPLANATION,
                            "This prevents NAVI from being uninstalled, ensuring focus sessions and app blocks cannot be bypassed."
                        )
                    }
        ...
        MethodChannel(flutterEngine.dartExecutor.binaryMessenger, "navi/app_blocker").setMethodCallHandler { call, result ->
            when (call.method) {
                ...
                "setBlockRules" -> {
                    ...
                    getSharedPreferences("app_blocker", Context.MODE_PRIVATE).edit()
                        .putStringSet("rules", rules.toSet())
                        .apply()
                    result.success(null)
                }
